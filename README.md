# 拓界搜索

拓界搜索根据用户输入和偏离度生成多个检索方向，再通过 SearXNG 聚合真实网页。SearXNG 不可用时，系统会明确切换到内置精选网址，保证演示不中断。

## 启动

当前机器需要先安装并启动 Docker Desktop。然后在本目录运行：

```powershell
docker compose -f searxng/docker-compose.yml up -d
$env:SEARXNG_URL="http://127.0.0.1:8080"
python server.py
```

访问 <http://127.0.0.1:8787/>。结果页底部会标明当前使用的是 `SearXNG 实时网页`、`SearXNG 实时网页 + 精选补充`，还是 `精选网址降级`。

如果 SearXNG 部署在其他机器，只需修改环境变量：

```powershell
$env:SEARXNG_URL="https://你的-searxng-地址"
python server.py
```

该实例必须在 `settings.yml` 的 `search.formats` 中启用 `json`。不建议把随机公共实例作为正式后端，它们通常禁用 JSON 接口或有严格限流。

默认中文检索会聚合百度和 Google，再按查询词命中程度重排。如果需要调整搜索源，可以设置：

```powershell
$env:SEARXNG_ENGINES="baidu,google"
$env:SEARXNG_FALLBACK_ENGINES="bing"
```

当百度或 Google 因验证码、限流而返回空结果时，后端会自动改用 Bing。对于无法识别且所有实时源均无结果的词，页面不会再用无关精选网址凑数。

多个偏离方向与备用搜索源都会并行检索；整次搜索默认最多等待 3.5 秒，相同查询默认缓存 120 秒。可通过 `SEARXNG_TIMEOUT`、`SEARCH_TOTAL_TIMEOUT` 和 `SEARCH_CACHE_TTL` 调整这些限制。

结果页每页显示 10 条，并采用按需扩充的动态分页：第一页只请求首批网页；当用户翻到尚未加载的页面时，后端才继续请求 SearXNG 的后续页，将新网址去重后追加进缓存池。连续两批没有新网址时停止显示“下一页”。默认安全上限为 50 页，可通过 `MAX_RESULT_PAGES` 调整；这是防止误操作和免费来源限流的保护值，并非固定结果数量。

## 测试

```powershell
python -m unittest -v
```
